database = dofile("./File_ibs/redis.lua").connect("127.0.0.1", 6379)
serpent = dofile("./File_ibs/serpent.lua")
JSON    = dofile("./File_ibs/dkjson.lua")
json    = dofile("./File_ibs/JSON.lua")
URL = dofile("./File_ibs/url.lua")
...
print("::SAKI::")
local RunBot = io.open("SAKI", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/SAKI
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr SAKI.lua
wget "https://raw.githubusercontent.com/SAKI-TEAM/SAKI/master/SAKI.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./SAKI.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
